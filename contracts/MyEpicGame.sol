...
contract MyEpicGame is ERC721 {
    ...
    ERC721("Heroes", "HERO")
  {
    ...
    console.log("Done initializiing boss %s w/ HP %s, img %s",bigboss.name, bigboss.hp, bigboss.imageURI);
  ...
  function attackBoss() external {
    uint256 playerId = nftHolders[msg.sender];
    CharacterAttributes storage player = nftHolderAttributes[playerId];
    console.log("\nPlayer w/ character %s about to attack. Has %s HP and %s AD", player.name, player.hp, player.attackDamage);
    console.log("Boss %s has %s HP and %s AD", bigboss.name, bigboss.hp, bigboss.attackDamage);

    require(player.hp > 0, "Error: Player must have some HP to attack boss");
    require(bigboss.hp > 0, "Error: Boss is already defeated");
